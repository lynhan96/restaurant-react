...
const EditFormInputText = (props) => {
  const { name, defaultValue, input, label, type = 'text' } = props
  ...
  return (
    <div className='form-group label-floating' style={{ marginTop: '0' }}>
      <label>{label}</label>
      <input type={type} className='form-control' name={name} required='true' defaultValue={defaultValue} onChange={onChange}/>
    </div>
  )
}
